...
def init_admin(owner: Signer, admin: Empty[Admin]):
    ...
    signer.transfer_lamports(pda_pubkey, amount)  # 將 SOL 轉移到 PDA
...
def token_mint(signer: Signer, BONK_mint: Empty[TokenMint], admin: Admin):
    ...
    amount = 2 * 1_000_000
    signer.transfer_lamports(pda_pubkey, amount)  # 向 Mint 帳戶轉移 SOL
...
def admin_token_account(signer: Signer, admin_account: Empty[TokenAccount], mint: TokenMint, admin: Admin):
    ...
    amount = 3 * 1_000_000
    signer.transfer_lamports(pda_pubkey, amount)  # 向 Token 帳戶轉移 SOL
